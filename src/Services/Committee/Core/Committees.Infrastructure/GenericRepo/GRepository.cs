namespace Committees.Infrastructure.GenericRepo
{
         ...
         where T : BaseEntity<Guid>
    {
        ...
        /// <summary>
        /// Retrieve all records based on a given condition with deleted Items
        /// </summary>
        /// <param name="where"></param>
        /// <returns></returns>
        public virtual IQueryable<T> GetAllWithDeletedItems(Expression<Func<T, bool>> where)
        {
            return _dbContext.Set<T>().AsNoTracking()
                .Where(where).AsQueryable();
        }
        ...
        /// <summary>
        /// Retrieve all records using Pagination with Condition with DeletedItems
        /// </summary>
        /// <param name="where"></param>
        /// <returns></returns>
        public virtual IQueryable<T> GetAllWithDeletedItems(Expression<Func<T, bool>> where, int pageNumber, int pageSize, ref ResponseDTO responseDto)
        {
            var data = _dbContext.Set<T>().AsNoTracking()
                .Where(where).AsQueryable();
            var items = data.Skip((pageNumber - 1) * pageSize).Take(pageSize).AsQueryable();
        ...
        public virtual IQueryable<T> GetAllWithDeletedItems(int pageNumber, int pageSize, ref ResponseDTO responseDto)
        {
            if (pageNumber <= 0)
            {
                pageNumber = 1;
        ...
        public virtual IQueryable<object> GetAll(Expression<Func<T, bool>> where, Expression<Func<T, object>> select)
        {
         ...
         public virtual IQueryable<object> GetAllWithDeletedItemsAsync(Expression<Func<T, bool>> where, Expression<Func<T, object>> select)
        {
            var x = _dbContext.Set<T>().AsNoTracking()
                .Where(where).Select(select);
            return x;
